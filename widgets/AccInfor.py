...
class AccInfor(CTkFrame):
    def __init__(self, master, corner_radius, change_account_click=None, *arg, **kw) -> None:
        ...
        self.label_avt: CTkLabel = CTkLabel(self, text="", width=int(
            self.height * 0.7), height=int(self.height * 0.7))
        ...
        name_text = CTkLabel(
            frame_name, textvariable=self.name, anchor="sw", font=font_name)
        ...
        img = CTkImage(load_img(r'assets\img\menu-d.png'),
                       load_img(r'assets\img\menu-l.png'))
        self.change_account = CTkButton(self, height=40, width=40, text='', image=img,
    ...
    async def get_avt_img_and_set(self, avt_url):
        ...
        self.avt_img = CTkImage(cropping_image_in_a_circular(
            img_pil), size=(int(self.height * 0.7), int(self.height * 0.7)))
