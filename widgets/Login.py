...
class Login(BaseMainFrame):
    def __init__(self, master, close_click, *arg, **kw):
        ...
        # button close
        close_img = CTkImage(load_img(r'assets\img\close-d.png'), load_img(f'assets\img\close-l.png'))
        self.close_button = CTkButton(self, image=close_img, command=close_click, text='', fg_color="transparent", height=40, width=40)
        
         # layout
        self.frame_center = CTkFrame(self)
        self.frame_center.place(anchor="c", relx=.5, rely=.5, relwidth=0.6, relheight=0.7)
        login_text = CTkLabel(
            self.frame_center, text="login", font=self.login_text_font)
        login_text.pack(pady=(20, 0))

        self.frame_user_name_pass_fill = CTkFrame(self.frame_center, fg_color="transparent")
        ...
        self.button = CTkButton(self.frame_center, text="LOGIN",height=35, command=button_command)
        self.button.pack(side=BOTTOM, pady=(0, 20))
        
    def _render_user_pass(self):
        if self.user_name_fill:
            self.user_name_fill.destroy()
        self.user_name_fill = CTkEntry(self.frame_user_name_pass_fill, placeholder_text="USERNAME", height=35, width=250, font=self.fill_font)
        self.user_name_fill.place(relx=.5, rely=.20, anchor='c')

        if self.user_pass_fill:
            self.user_pass_fill.destroy()
        self.user_pass_fill = CTkEntry(self.frame_user_name_pass_fill, placeholder_text="PASSWORD", height=35, width=250, font=self.fill_font)
