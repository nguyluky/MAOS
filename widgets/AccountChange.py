...
class AccView(CTkFrame):
    def __init__(self, master, corner_radius, name='', avt=URL_PLYER_CARD_DEF, title='',command_login=None, command_remove=None ,*arg, **kw) -> None:
        ...
        name_text = CTkLabel(frame_name, textvariable=self.name, anchor="sw", font=font_name, width=250)
...
class AccountChange(BaseMainFrame):
    def __init__(self, master,corner_radius ,close_click = None, add_click=None,*arg, **kw):
        ...
        self.frame_center = CTkScrollableFrame(self, corner_radius=corner_radius)
